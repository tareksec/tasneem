    ...
    <div className="py-12 sm:py-16 bg-white text-[#0A0A0A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <Link
            href="/machines"
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#4B5563] hover:text-[#0A0A0A] transition-colors"
          >
        ...
        <MotionSection className="border border-[#E5E7EB] rounded-2xl bg-[#F9FAFB] p-6 sm:p-10 mb-12 shadow-sm">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-[#E5E7EB] text-xs font-bold text-[#0A0A0A] mb-4">
            <Gauge className="w-3.5 h-3.5 text-[#FF0000]" />
            <span>Typical Range: {catInfo.typicalGauge}</span>
          </div>

          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-[#0A0A0A]">
            {catInfo.name}
          </h1>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#D1D5DB] bg-white hover:border-[#FF0000] text-xs font-semibold text-[#0A0A0A] transition-colors"
                  >
                    <span>{sub.label}</span>
          ...
          <div className="mt-6 pt-6 border-t border-[#E5E7EB]">
            ...
            <div className="flex flex-wrap gap-2">
              {catInfo.commonApplications.map((app) => (
                <span
                  key={app}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#E5E7EB] bg-white text-xs font-medium text-[#0A0A0A]"
                >
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            ...
            className="bg-[#FF0000] hover:bg-[#E00000] text-white px-4 py-2 rounded-lg text-xs font-semibold inline-flex items-center gap-1.5 shadow-sm transition-colors"
          >
        ...
        {machines.length > 0 ? (
          ...
          <div className="border border-dashed border-[#E5E7EB] rounded-xl p-12 text-center bg-[#F9FAFB]">
            ...
            <Link
              href={`/quote?category=${catInfo.slug}`}
              className="mt-4 inline-block bg-[#FF0000] hover:bg-[#E00000] text-white px-5 py-2 rounded-lg text-xs font-semibold transition-colors"
            >
        ...
        <MotionSection delay={0.1} className="mt-16 pt-12 border-t border-[#E5E7EB]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {CATEGORIES.filter((c) => c.slug !== catInfo.slug).map((c) => (
              ...
              >
                <span className="font-bold text-xs text-[#0A0A0A]">{c.name}</span>
